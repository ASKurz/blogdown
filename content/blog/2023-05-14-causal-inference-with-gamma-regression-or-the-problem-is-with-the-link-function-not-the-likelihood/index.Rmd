...
brm2 <- brm(
  data = horan1971,
  family = Gamma(link = "log"),
  bf(post ~ 0 + Intercept + experimental + prelc,
     shape ~ 0 + Intercept + experimental + prelc),
  prior = prior(normal(5.053056, 0.09562761), class = b, coef = Intercept) +
    prior(normal(0, 0.25), class = b, coef = experimental) +
    prior(normal(0.75, 0.25), class = b, coef = prelc),  # the brms default
  cores = 4, seed = 6
)

# pp-check
set.seed(6)
p4 <- pp_check(brm2, type = "stat_grouped", group = "experimental", stat = "mean") +
  labs(subtitle = "means") +
  coord_cartesian(xlim = c(143, 162))

set.seed(6)
p5 <- pp_check(brm2, type = "stat_grouped", group = "experimental", stat = "sd") +
  labs(subtitle = "SD's") +
  coord_cartesian(xlim = c(9, 26))

set.seed(6)
p6 <- pp_check(brm2, type = "stat_grouped", group = "experimental", stat = "skewness") +
  labs(subtitle = "skews") +
  coord_cartesian(xlim = c(-1, 2))
...
It's a thing of beauty, isn't it? For more details on how to use **marginaleffects** functions to work with `brm()` models, check out Arel-Bundock's [-@arelBundock2023BayesianAnalysis] vignette, *Bayesian analysis with brms*. To see how to make the same computation with a `fitted()`- or `add_epred_draws()`-based workflow, go back to the [fourth post](https://solomonkurz.netlify.app/blog/2023-04-30-causal-inference-with-bayesian-models/) in this series.
...
[^1]: It isn't totally under-used, though. Some researchers have found gamma regression (with the log link) useful for modeling healthcare cost data [@basu2004comparing; @malehi2015statistical]
...
[^3]: The language of "quirks" is my own. The inverse link has the technical limitation that it will not insure on its own that the model will not return negative predictions, which is an insight you can find in the technical literature [e.g., @mccullagh1989generalized]. In addition, I have personally found the inverse link can cause estimation difficulties with both frequentist (`glm()`) and Bayesian (`brm()`) software. The log link *just works*, friends. Use the log link for gamma.

[^4]: You don't have to believe me. Check it for yourself. You might do a visual check with a histogram or density plot. Or you could compute the sample skewness statistic with the `skew()` function from the **psych** package [@R-psych].
