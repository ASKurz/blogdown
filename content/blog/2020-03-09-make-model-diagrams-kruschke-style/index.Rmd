...
```{r, fig.width = 2, fig.height = 1.25, warning = F}
...
```{r, fig.width = 2, fig.height = 1, warning = F}
p1 <-
  tibble(x = seq(from = .01, to = .99, by = .01),
       d = (dbeta(x, 2, 2)) / max(dbeta(x, 2, 2))) %>% 
  ggplot(aes(x = x, y = d)) +
...
```{r, fig.width = 4, fig.height = 1, warning = F}
p2 <-
...
ggsave("kruschke-diagram-featured.jpg", width = 3, height = 5, units = "in")
```
