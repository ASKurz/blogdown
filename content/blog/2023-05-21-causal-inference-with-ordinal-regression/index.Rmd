...
For identification purposes, the $\beta_0$ parameter is fixed to 0, which I have explicitly denoted in the third line of the equation. We have set $\alpha$ to 1, also for identification purposes. In case you're not familiar, $\alpha$ is the *discrimination* parameter, which is the inverse of $\sigma$. Bürkner uses this parameterization in **brms** functions because of its connection to item response theory [see., @burknerBayesianItemResponse2020; @burknerOrdinalRegressionModels2019].

In this blog post, we'll include our predictor variables in a linear model for the latent mean $\mu_i$. In principle, they could be used in other ways, such as with category-specific effects. You can learn about those and other useful alternatives in Bürkner and Vuorre's nice [-@burknerOrdinalRegressionModels2019] tutorial paper. In our models, we will allow our focal independent variable `protection`to influence the model with the $\beta_1$ parameter.
...
Ordinal models can make causal inference trickier than it is with other kinds of models. @volfovsky2015causal introduced the topic well:
  
> The first step when attempting any causal analysis is the choice of an estimand, or inferential target, which is the object of interest. When outcomes are continuous, the most commonly studied estimand is the average treatment effect [e.g., see @holland1986statistics; @rubinEstimatingCausalEffects1974]. However, this quantity is not well defined for non-numeric data as the notion of an average of more than two ordinal values is not well defined. Other measures of centrality might be of interest; for example, the modal difference between treatment and control describes the most common number of categories that are changed due to treatment. (p. 4)
...
* $\tau_z^\text{ATE}$, the average treatment effect for latent potential outcomes [@volfovsky2015causal];
* $\Delta_j^\text{ATE}$, the difference in probability for each of the $J$ Likert response options [@boes2013nonparametric]; and
...
Sometimes we think of ordinal data as coarse measurements of a continuous underlying latent variable [@burknerOrdinalRegressionModels2019; @gelmanRegressionOtherStories2020, Chapter 13]. In the case of our cumulative probit models, the latent variable is expressed as a standard normal distribution, which we might call $Z$.[^5] Thus instead of the conventional formula
...
We can take very different approach by computing a treatment effect defined in terms of the probabilities of the given Likert response options. @boes2013nonparametric referred to this effect size as $\Delta_y^\text{ATE}$. Frankly, I find Boes's notation slightly opaque, and I'm going to refer to his version of the ATE throughout this post as $\Delta_{\color{blueviolet}{j}}^\text{ATE}$, which is meant to more explicitly connect the effect size with a given level of the $J$ ordered response options. We might define $\Delta_j^\text{ATE}$ as
...
```{r, fig.width = 8, fig.height = 5.5, message = F}
# conditional expectations
...
In the case of the cumulative probit ANCOVA, the $y_i^1 - y_i^0$ contrasts do differ across participants, albeit not much in the case of the data from @coyne2022awareness.
...
```{r, eval = F, echo = F}
# compare SD's
0.1109938 / 0.06964341
0.06964341 / 0.1109938
```
...
We've already covered enough estimands for one blog post. But researchers have proposed other options. To learn about more, read through @boes2013nonparametric, @volfovsky2015causal and @chiba2018bayesian, and look through their reference sections. It's still the Wild West in this part of the literature.
...
[^4]: If you're disturbed by how imperfectly the models captured the sample data, consider fitting models with category-specific effects. This is currently allowed for `family = cumulative(probit)` models as an experimental feature of the `cs()` function. You can learn more about `cs()` in the *Special predictor terms* subsection of the `brmsformula` section of the **brms** reference manual [@brms2023RM].

[^5]: In some parts of the literature, the ordinal variable is termed $Y$ and the continuous underlying latent variable is termed $Y^*$. However, @volfovsky2015causal favored the $Z$ notation. Plus, nobody in their right mind would want to write out $\tau_{y^*}^\text{ATE}$ for their causal estimand.
...
```{r, eval = F, echo = F}
# save the color
v <- viridis::viridis_pal(option = "mako")(10)

# pull the cumulative probabilities
cum_pr_k <- coyne2022 %>%
  count(post) %>%
  mutate(pr_k     = n / sum(n),
...
# plot
tibble(z = seq(from = -6.5, to = 6.5, length.out = 4e3)) %>% 
  mutate(d = dlogis(x = z, location = 0, scale = 1),
         c = case_when(
           z < qlogis(p = cum_pr_k[1]) ~ "a",
           z < qlogis(p = cum_pr_k[2]) ~ "b",
           z < qlogis(p = cum_pr_k[3]) ~ "c",
           z < qlogis(p = cum_pr_k[4]) ~ "d",
           z < qlogis(p = cum_pr_k[5]) ~ "e"
         )) %>% 
  
  ggplot(aes(x = z, y = d)) +
  geom_line(color = v[5], linewidth = 1) +
  geom_area(aes(fill = c)) + 
  geom_vline(xintercept = qlogis(p = cum_pr_k[1:4]), linetype = 3) +
  scale_fill_viridis_d(option = "mako", direction = -1, begin = .3, end = .95, breaks = NULL) +
  scale_x_continuous("latent logistic distribution", breaks = -2:2 * 3,
                     sec.axis = dup_axis(
                       name = NULL,
                       breaks = qlogis(p = cum_pr_k[1:4]),
                       labels = parse(text = str_c("tau[", 1:4, "]"))
                     )) +
  scale_y_continuous(NULL, breaks = NULL) +
  coord_cartesian(xlim = c(-5.8, 5.8)) +
  theme_gray(base_size = 13) +
  theme(panel.grid = element_blank(),
        panel.background = element_rect(fill = "#eaf0f5")) # alpha(v[5], 1/20)

# "#eaf0f5" is close to alpha(v[5], 1/20) 
# see https://www.color-hex.com/color/366a9f

ggsave("latent-featured.jpg", width = 3.6, height = 3.5, units = "in")
```
