...
```{r, echo = F}
# save(b0, file = "fits/b0.rda")
# save(b1, file = "fits/b1.rda")
# save(b2, file = "fits/b2.rda")
# save(b3, file = "fits/b3.rda")
# save(b4, file = "fits/b4.rda")
# save(b1.1, file = "fits/b1.1.rda")
# save(b1.2, file = "fits/b1.2.rda")
...
```{r loo_b0_loo_b1, message = F, warning = F, results = 'hide'}
loo_b0 <- loo(b0)
loo_b1 <- loo(b1)
```
...
```{r loo_b2_loo_b4, message = F, warning = F}
loo_b2 <- loo(b2)
loo_b3 <- loo(b3)
loo_b4 <- loo(b4)
```
...
```{r, fig.width = 3.5, fig.height = 3.25, eval = F, echo = F}
tibble(x = seq(from = -6, to = 6, by = .01)) %>% 
  expand(x, nu = c(1, 2.5, 5, 10, Inf)) %>% 
  mutate(density = dt(x = x, df = nu),
         nu      = factor(nu, levels = c("Inf", "10", "5", "2.5", "1"))) %>% 
  ...
  theme(legend.background = element_rect(fill = "grey95"),
        ...
        legend.position = c(.88, .73),
        panel.grid = element_blank())

ggsave("t-densities-featured.jpg", width = 4, height = 3.5, units = "in")
```
