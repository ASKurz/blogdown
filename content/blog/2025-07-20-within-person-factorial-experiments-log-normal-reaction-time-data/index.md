...
This is the first in a new series discussing causal inference with experimental data using multilevel models.
...
nd <- stroop_subset |> 
  ...
  expand_grid(cond = levels(stroop_subset$cond) |> 
                factor(levels = levels(stroop_subset$cond)))
...
In late 2023 **lme4** began offering an `se.fit = TRUE` method for the `predict()` function. If you execute `?predict.merMod` for the documentation, you will find this method is currently considered “experimental”. There is no current direct method to request 95% CIs, but you can compute Wald intervals by hand. Exciting as it is, the `se.fit = TRUE` method won’t actually serve us well here because we’re interested in averaging across the `trial`-level estimates with `subject`s, and we’ll be interested in computing within-`subject` contrasts from them as well.
