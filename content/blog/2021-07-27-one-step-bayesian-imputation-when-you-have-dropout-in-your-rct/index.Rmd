...
```{r d_plot, fig.width = 6, fig.height = 1.75, warning = F}
levels <- c(
...
```{r echo = FALSE}
blogdown::shortcode('tweet', user = "SolomonKurz", id = '1420434272142434304')
```
...
posterior_summary(fit2) %>% 
  ...
  # plot!
  ggplot(aes(x = pre, y = Estimate, ymin = Q2.5, ymax = Q97.5, color = tx)) +
  geom_pointrange(fatten = 1/2, linewidth = 1/6) +
  scale_color_viridis_d(NULL, option = "F", begin = .2, end = .6, direction = -1) +
  ylab("post (imputed)") +
  coord_equal(xlim = c(1.5, 8.5),
              ylim = c(1.5, 8.5)) +
...
ggsave("one-step-featured.jpg", width = 3.6, height = 3.5, units = "in")
```
