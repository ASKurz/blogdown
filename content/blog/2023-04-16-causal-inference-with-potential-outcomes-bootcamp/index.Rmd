...
In post, we'll be continuing on with our `horan1971` data set form the last post. These data, recall, were transposed from the values displayed in Table 2 from @horan1971coverant. I've saved them as an external `.rda` file on GitHub ([here](https://github.com/ASKurz/blogdown/tree/main/content/blog/2023-04-12-boost-your-power-with-baseline-covariates/data)). If you don't want to download a data file from my GitHub, you can just copy the code from the [last post](https://solomonkurz.netlify.app/blog/2023-04-12-boost-your-power-with-baseline-covariates/).
...
[^6]: I'm still getting my footing in this literature, so please forgive any mistakes in my documentation. To my knowledge, *g-computation* has its origins in the lengthy technical work by @robins1986new, an epidemiologist interested in causal inference with observational data. The practice of *standardization* (which does not mean computing $z$ scores in this context) has a long history among epidemiologist and demographers. For a historical and methodological overview of *standardization* for computing death rates within epidemiology, see @keiding1987method. I believe statisticians Lane and Nelder [-@lane1982analysis] were the first to propose *standardization* as a post-processing step for ANCOVA-type GLM's fit to data from experiments. Fun fact: Nelder was the principle author of, you know, the *Generalized Linear Model* [@nelder1972generalized]. No big deal.
...
[^8]: It's a big deal how **marginaleffects** so easily returns standard errors and their derivatives (e.g., 95% CI's, $p$-values) for the $\mathbb E (\hat y_i^1 - \hat y_i^0 \mid c_i)$ via the *standardization*/*g-computation* approach. Even pretty recent frequentist tutorials have relied on bootstrapping for standard errors and 95% intervals [e.g., @hernan2020CausalInference; @wang2017gcomputation]. Bootstrapping methods are great in a pinch, but this is so much better. If you'd like to learn more, Arel-Bundock's [-@arelBundock2023CausalInference] vignette shows a comparison of the results from his `avg_comparisons()` function to those from bootstrapping for one of the examples in Hernán and Robins's [-@hernan2020CausalInference] textbook.
...
# ggdark alternative
(p3 +
   scale_color_viridis_d(NULL, option = "mako", begin = .6, end = .9,
                         labels = scales::parse_format()) +
   ylab(NULL)) +
  (p4 +
     geom_vline(xintercept = 0, color = "gray33") +
     geom_interval(aes(xmin = conf.low, xmax = conf.high),
                   size = 1/5, color = "white")) + 
  plot_annotation(title = "Person-level estimates from the ANCOVA") &
   ggdark::dark_theme_gray(base_size = 10.2) +
   theme(panel.grid = element_blank(),
         plot.margin = margin(l = 2, r = 2, t = 5, b = 5),
         plot.title = element_text(size = 12.6),
