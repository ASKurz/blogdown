...
We'll be working with a subset of the `epilepsy` data from the **brms** package. Based on the **brms** documentation (execute `?epilepsy`), the data have their proximal origins in a [-@thall1990some] *Biometrics* paper by Thall and Vail. However, Thall and Vail cited an earlier paper by @leppik1985double as the ultimate origin.[^1] Here we load our primary packages, adjust the global plotting theme, load the `epilepsy` data, and take a peek.
